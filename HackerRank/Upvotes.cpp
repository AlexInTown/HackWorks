...
int main() {
    ...
    for(i = 0; i + K < N; ++i){
        if(leftDescCnt==1){
            for(j = i+1; j < i+K && arr[j]>=arr[j-1]; ++j);
            leftAscCnt = j-i;
        }
        if(leftAscCnt==1){
            for(j = i+1; j < i+K && arr[j]<=arr[j-1]; ++j);
        ...
        if(arr[i] >= arr[i-1] && rightAscCnt <= K) {leftAscCnt--; sum-=leftAscCnt;}
        if(arr[i] <= arr[i-1] && rightDescCnt <= K) {leftDescCnt--; sum+=leftDescCnt;}
        if(rightAscCnt > K) rightAscCnt = K;
        if(rightDescCnt > K) rightDescCnt = K;
        if(rightAscCnt == K && rightDescCnt==K) sum = 0;
        ...
        /*if(rightAscCnt > K){
            rightAscCnt = K; leftAscCnt++;
        }
        if(rightDescCnt> K){
            rightDescCnt = K; leftDescCnt++;
        }
        
        printf("leftAsc=%d leftDesc=%d rightAsc=%d rightDesc=%d\n", leftAscCnt, leftDescCnt, rightAscCnt,  rightDescCnt);
        if(rightAscCnt < K) sum -= leftAscCnt;
        sum += rightAscCnt-1;
        if(rightDescCnt < K) sum += leftDescCnt;
        sum -= rightDescCnt-1;
